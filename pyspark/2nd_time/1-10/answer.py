...
# **`truncate=False`**は、列の内容が表示される際に長すぎる場合に省略されないようにします。
...
# メモ
# Rubyのhashのような形でカラムを指定できる。`df_receipt['customer_id']`
...
print('P-007 ----------')
df_receipt.select(["sales_ymd", "customer_id", "product_cd", "amount"])\
          .filter((df_receipt['customer_id'] == 'CS018205000001') & (df_receipt['amount'] >= 1000) & (df_receipt['amount'] <= 2000)).show()


# メモ
# F.col("customer_id")で指定する方が一般的？
